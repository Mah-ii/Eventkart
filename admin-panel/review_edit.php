    ...
    <div>
        <div class="fromCon">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="fromConn">
                <input type="text" name="name" placeholder="Your Name" value="<?php echo $row2['user_name'] ?>"
                    required><br>
                <input type="email" name="email" placeholder="Your Email" id=""
                    value="<?php echo $row2['user_email'] ?>" required><br>
                <textarea name="review" placeholder="Review" id="" cols="30" rows="10"
                    required><?php echo $row2['review'] ?></textarea>
                <input type="file" name="image" placeholder="Image" id=""><br>
                <input type="hidden" name="id" value="<?php echo $row2['id'] ?>">
                <select class="formCon" id="category" name="category">
                    <option value="Wedding" <?php if ($row2['category'] == "Wedding")
                        echo "selected" ?>>Wedding</option>
                        <option value="Birthday" <?php if ($row2['category'] == "Birthday")
                        echo "selected" ?>>Birthday
                        </option>
                    </select>
